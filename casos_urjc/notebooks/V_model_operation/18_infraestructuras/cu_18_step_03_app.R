...
dmap <- distritos_geojson() |> left_join(datos_distritos(), by = c("CDIS" = "cdis"), multiple="all") #|> slice(1:4000)
...
dmap |> leaflet() |>
  ...
  addLegend("bottomright", pal = qpal, values = dmap[[input$variable_plot]],
    ...
    dmap <- df_summarised_diario() |> left_join(datos_infra(), by = c("id_inf" = "id_inf"), multiple="all") |> slice(1:4000)
